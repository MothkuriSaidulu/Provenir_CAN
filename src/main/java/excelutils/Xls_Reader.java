...
public class Xls_Reader {
	...
	public static String filename = "src\\config\\testcases\\TestData.xlsx";
...
//Added for Test Data workbook on 03-MAY-2019 ENDs
    ...
    public static TreeMap<String, String> getCellDataBasedOnRowValue(String sheetName, String senarioName) throws Exception
    {
    	...
    	try
    	{
	    	...
	    	for (int column = (firstColumnNumber + 1); column <= lastColumnNumber-1; column++)
	    	{
	        	...
	        	{
	        		//System.out.println("P :"+p); Commented on 19-Mar-2022
	        		row = sheet.getRow(p);
 ...
 //Added on 06-MAY-2019 Start : for getting test data from separate Workbook-> Work sheet  
  ...
  //Added on 06-MAY-2019 End : for getting test data from separate Workbook-> Work sheet
    ...
    public boolean setCellDataForTestData_online(String sheetName,String colName,int ColNum, String data){
        try{
            ...
            int lastrow = sheet4TestData.getLastRowNum();
            ...
            for(int i=0;i<=lastrow;i++){
            	 row=sheet4TestData.getRow(i);
       	    	 //System.out.println("====");
            	 //System.out.println(i);
                 //System.out.println(row.getCell(0).getStringCellValue().trim());
                 if(row.getCell(0).getStringCellValue().trim().equals(colName))
                 {
                	 colNum=i;
	...
	/*
	 * public boolean setCellDataForTestData_online_DB(String sheetName,String
	 * colName,int ColNum, List<String> data){ try{
	 * 
	 * int noOfTestSet = 0, lastColumnNumber; //private variable declarations int
	 * noofrows = 0;
	 * 
	 * fis4TestData = new
	 * FileInputStream(TestBase.rootPath+"\\ExcelData\\TestData.xlsx");
	 * workbook4TestData = new XSSFWorkbook(fis4TestData);
	 * 
	 * if(ColNum<=0) return false;
	 * 
	 * int index = workbook4TestData.getSheetIndex(sheetName); int colNum=-1;
	 ...
	 * int lastrow = sheet4TestData.getLastRowNum();
	 * 
	 * int rowcount = 0; try { rowcount = getRowCount(sheetName); } catch (Exception
	 * e) { // TODO Auto-generated catch block e.printStackTrace(); }
	 * 
	 * System.out.println("ROW COUNT"+rowcount);
	 * System.out.println("LAST ROW NUM"+lastrow);
	 * 
	 * for(int i=0;i<=lastrow;i++){ row=sheet4TestData.getRow(i);
	 * //System.out.println("===="); //System.out.println(i);
	 * //System.out.println(row.getCell(0).getStringCellValue().trim());
	 * if(row.getCell(0).getStringCellValue().trim().equals(colName)) { colNum=i;
	 * System.out.println("FINAL"); System.out.println(colNum); break; } } cell =
	 * row.getCell(ColNum); cell.setCellValue(data); fileOut = new
	 * FileOutputStream(TestBase.rootPath+"\\ExcelData\\TestData.xlsx");
	 * workbook4TestData.write(fileOut); fileOut.close();
	 * System.out.println("DONE DONE DONE"); } catch(Exception e){
	 * e.printStackTrace(); return false; } return true; }
	 * 
	 * public boolean setCellDataForTestData_online_DB1(String sheetName,String
	 * colName,int ColNum, HashMap<String, String> data){ try{
	 * 
	 * int noOfTestSet = 0, lastColumnNumber; //private variable declarations int
	 * noofrows = 0;
	 * 
	...
	try	  
		{
          ...
          //System.out.println(sheetName+ " : is not available in the Excel");
          else
          sheet = workbook.getSheetAt(index);
	...
	public boolean setHeader(String sheetName) 
	{
		try
		{
			logger.info("Creating empty sheet for report");
	         ...
	         header.createCell((short) 0).setCellValue("Feature_Name");
	         header.createCell((short) 1).setCellValue("Feature_Id");
	         header.createCell((short) 2).setCellValue("Feature_Description");
	         header.createCell((short) 3).setCellValue("Scenario_Id");
	         header.createCell((short) 4).setCellValue("Scenario_Description");
	         header.createCell((short) 5).setCellValue("Browser");
	         ...
	         header.createCell((short) 5).setCellValue("TestCaseRemarks");
	         header.createCell((short) 7).setCellValue("ExecutedAt");
	         fileOut = new FileOutputStream(path);
	         workbook.write(fileOut);
	         fileOut.close();
	    	return true;
	...
	public void generatePivotReport(String sheetName) throws IOException
	{
         ...
         //XSSFPivotTable pivotTable = sheet.createPivotTable(aref, pos, sheet); 
    //pivotTable.addRowLabel(3);
         //pivotTable.addRowLabel(9);
         //pivotTable.addColumnLabel(DataConsolidateFunction.COUNT_NUMS,
           //            9, "Count of Status");
...
/*
    ...
    public static void main(String arg[]) throws IOException{
        ...
        datatable = new Xls_Reader("D:/Selenium/WS/USFDADataScience_rnd/ExcelData/AppiumReportFormat.xlsx");
